...
class DashletCalendar extends Dashlet
{
	...
	{
		$this->sModuleName = utils::GetCurrentModuleDir(0);
		$this->sAjaxUrl = utils::GetAbsoluteUrlModulesRoot().utils::GetCurrentModuleDir(0).'/ajax.php';
	...
	static public function GetInfo()
	{
		return array(
			...
			'icon' => 'env-'.Utils::GetCurrentEnvironment().'/'.utils::GetCurrentModuleDir(0).'/images/timetable32.png',
	...
	public function Render($oPage, $bEditMode = false, $aExtraParams = array())
	{
		...
		$sTimeFormat = utils::GetConfig()->GetModuleSetting($this->sModuleName, 'time_format', 'H:mm');
...
EOF
	...
	public function GetPropertiesFields(DesignerForm $oForm)
	{
		...
		$oField = new DesignerComboField('default_view', Dict::S('UI:DashletCalendar:Prop-Default-View'),
			$this->aProperties['default_view']);
		$oField->SetMandatory();
		...
		// Agenda week view
		$oField = new DesignerBooleanField('agenda_week', Dict::S('UI:DashletCalendar:Prop-Agenda-Week'),
			$this->aProperties['agenda_week']);
		$oForm->AddField($oField);
...
class DashletCalendarStaticContent implements iPageUIExtension, iApplicationUIExtension
{
	...
	public static function AddToPage(WebPage $oPage)
	{
		if (!self::$bLoaded)
		{
			$sModuleUrlBase = utils::GetAbsoluteUrlModulesRoot().utils::GetCurrentModuleDir(0);
